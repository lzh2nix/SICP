...
(define (div-terms L1 L2)
  (if (empty-term-list? L1)
      ...
      (let ((t1 (first-term L1))
            (t2 (first-term L2)))
        (if (great? (order t2) (order t1))
            (list the-empty-term-list L1)
            (let ((new-c (div (coeff t1) (coeff t2)))
                  (new-o (sub (order t1) (order t2))))
              (let ((rest-of-resut (div-terms (sub-terms L1
                                                         (mul-terms-by-all-terms (make-term new-o new-c) L2))
                                              L2)))
                (list (adjoin-term (make-term new-o new-c) (car rest-of-resut)) (cadr rest-of-resut))))))))
(define (remainder-terms a b)
  (let ((rem (cadr (div-terms a b))))
    (if (null? rem)
        the-empty-term-list
        (if (less? (order (car rem)) (make-scheme-number 1))
            (adjoin-term (make-term (make-scheme-number 0)  (make-scheme-number 1)) the-empty-term-list)
            rem))))
